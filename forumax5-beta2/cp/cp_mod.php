...
<table width="100%">
 <tr>
  ...
  <td width="80%"><select name="the_forum"><?php BuildForumList ($result2[1]); ?></td>
 <tr>
   <td>Start date</td>
...
<?php
    ...
    if ((IsMod ($GLOBALS["userid"]) == 0) and (IsCategoryMod ($GLOBALS["userid"]) == 0) and (($GLOBALS["flags"] & FLAG_ADMIN) == 0) and (($GLOBALS["flags"] & FLAG_MMOD) == 0)) {
	// no. complain
	die ("You're not a moderator");
    }
