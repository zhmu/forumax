<?php
    ...
    // define (NEWESTVERSION_URL, "http://cu/forumax/updates/curver.txt");
    ...
    // define (GRABFILE_URL, "http://cu/forumax/updates/getfile.php?username=%s&password=%s&file=%s");
...
<input type="checkbox" name="smod">User must be a <b>Super Moderator</b></input><br>
<input type="checkbox" name="mod">User must be a <b>Moderator</b></input><br>
...
<?php
	...
	if ($type != "all") {
	    ...
	    if ($smod != "") { $smod_op = "!="; } else { $smod_op = "=="; };
	    if ($mod != "") { $mod_op = "!="; } else { $mod_op = "=="; };
	    $set = 0;
...
<?php
	...
	// build the query
	$query = sprintf ("update accounts set %saccountname='%s',password='%s',email='%s',nofposts='%s',parent_email='%s',parent_password='%s',sig='%s',flags=%s where id=%s",$newcustom,$the_accountname,$the_password,$the_email,$the_nofposts,$the_parent_email,$the_parent_password,$the_sig,$flags,$accountid);
	db_query ($query);
...
<?php
	...
	while ($result = db_fetch_results ($res)) {
	    ...
	    $export .= "\$SKIN[\"" . htmlspecialchars ($result[0]) . "\"]=\"" . htmlspecialchars ($result[1]) . "\";\n";
	    $export .= "\$SKINTITLE[\"" . htmlspecialchars ($result[0]) . "\"]=\"" . htmlspecialchars ($result[2]) . "\";\n";
	    $export .= "\$SKINREFRESH[\"" . htmlspecialchars ($result[0]) . "\"]=\"" . htmlspecialchars ($result[3]) . "\";\n";
	}
...
<table width="100%">
 ...
 <tr>
  ...
  <td><textarea rows=10 cols=50 name="skinimport"></textarea></td>
 </tr>
</table><p>
...
<?php
	...
	$skinimport = preg_replace ("/\\$/", "\\\$", $skinimport);

	// but change all \$SKIN thingies back to $SKIN
	$skinimport = preg_replace ("/\\\\\\\$SKIN/", "\$SKIN", $skinimport);
	...
	while (list ($name, $content) = each ($SKIN)) {
	    ...
	    $query = sprintf ("insert into skin_%s values ('%s','%s','%s','%s')", $id, $name, $content, $SKINTITLE[$name], $SKINREFRESH[$name]);
	    db_query ($query);
	}
...
<?php
	...
	// add the category
	$query = sprintf ("insert into categories values (NULL,'%s',0)", $catname);
	db_query ($query);
...
<?php
	...
	$fp = fopen (NEWESTVERSION_URL, "rb") or die (" Failure</li></ul>");
...
<?php
 ...
 ?>The forum has successfully been updated. You are now the proud owner of ForuMAX <b><?php echo $new_version; ?></b>, instead of <?php echo FORUMAX_VERSION ?>. We hope you feel much better now :)
<?php
	}
