<?php
	...
	if ($how == "lock") {
	    ...
	    $query = sprintf ("insert into threads values (NULL,%s,'%s',%s,%s,now(),%s,%s,'%s','','',0)",$destforum,$title,$icon,$nofreplies,$authorid,$flags,$lastposterid, $nofviews);
	    db_query ($query);
		...
		// re-post it to the new forum
	 	$query = sprintf ("insert into posts values (NULL,%s,%s,%s,'%s','%s','%s',%s,%s,'%s',%s)",$result[0],$destforum,$new_threadid,$result[1],$result[2],$result[3],$result[4],$result[5],$result[6],$result[7]);
	        db_query ($query);
	    }
