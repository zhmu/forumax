<?php
	...
	$query = sprintf ("select sig,timediff,date_format(birthday,'%%Y'),date_format(birthday,'%%c'),date_format(birthday,'%%e'),flags from accounts where id=%s", $GLOBALS["userid"]);
	...
	// update the entry
	$query = sprintf ("update accounts set password='%s',sig='%s',timediff='%s',birthday='%s',flags=%s where accountname='%s'", $newpassword1, $the_sig, $timezone, $date, $flags, $the_accountname);
	db_query ($query);
	...
	// use a new cookie now
        SetCookie ("authid", $the_accountname . ":" . $newpassword1, 3600);

	// it worked. show the 'wohoo' page
	ShowHeader("editprofile_ok");
	eval ("\$tmp = stripslashes (\"" . addslashes (GetSkinTemplate ("editprofile_ok")) . "\");");
	print $tmp;
	ShowFooter();
    }
 ?>
