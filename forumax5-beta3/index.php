<?php
	...
	$mod_splitemplate = AddSlashes (GetSkinTemplate ("list_splitmod"));
	$mod_splitemplate = AddSlashes (GetSkinTemplate ("list_splitmod"));
	$lastpost_template = AddSlashes (GetSkinTemplate ("lastpost"));
	$nolastpost_template = AddSlashes (GetSkinTemplate ("lastpost_none"));
	...
	while ($result = db_fetch_results ($res)) {
		...
		eval ("\$tmp = stripslashes (\"" . $lastpost_template . "\");");
		$lastpost = $tmp;
	    } else {
		// no. use the 'lastpost_none' template
		$lastpost = $nolastpost_template;
	    }
    ...
    BuildCatForumList() {
	// grab the template
	$catforumlist_cat_template = addslashes (GetSkinTemplate ("catforum_cat_list"));
	$catforumlist_forum_template = addslashes (GetSkinTemplate ("catforum_forum_list"));
	$lastpost_template = AddSlashes (GetSkinTemplate ("lastpost"));
	$nolastpost_template = AddSlashes (GetSkinTemplate ("lastpost_none"));
	$catforumlist = "";

	// build the query
	$query = sprintf ("select id,name,orderno from categories");
	$res = db_query ($query);

	// handle all categories
	while ($result = db_fetch_results ($res)) {
	    // get the data
	    $catid = $result[0]; $catname = $result[1]; $catorder = $result[2]; $catmods = "";

	    // format the template
	    eval ("\$tmp = stripslashes (\"" . $catforumlist_cat_template . "\");");
                 $catforumlist .= $tmp;
	    ...
	    $query = sprintf ("select id,name,nofposts,nofthreads,from_unixtime(unix_timestamp(lastpost)+%s),lastposterid,description from forums where catno=%s order by orderno asc",$timezone, $catid);
	    $res2 = db_query ($query);
                 while ($result = db_fetch_results ($res2)) {
	         ...
	         if ($nofposts != 0) {
	            ...
	            $lastpost = $nolastpost_template;
                      ...
                      // format the template
                      eval ("\$tmp = stripslashes (\"" . $catforumlist_forum_template . "\");");
                      $catforumlist .= $tmp;
                 }
	 }
	...
	if ($catid == "") {
	    ...
	    if ($CONFIG["intro_type"] == 2) {
	        $template = "welcome_catforum";
                     ...
                     $catforumlist = BuildCatForumList();
	    }
	    if ($CONFIG["intro_type"] == 2) {
	        $template = "welcome_catforum";
	    }
