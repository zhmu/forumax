...
<?php
	...
	global $master_pwd1, $master_pwd2;
	...
	while (list ($name, $content) = each ($SKIN)) {
	    ...
	    $query = sprintf ("insert into skin_%s values ('%s','%s','%s','%s')", $skinid, $name, $content, $title, $refresh);
	    db_query ($query);
	}
	...
	// step 20: create backup directories
