<?php
    ...
    define (FORUMAX_VERSION, "5.0 BETA 3");
	...
	if ($GLOBALS["skinid"] == 0) {
	    ...
	    $query = sprintf ("select id from skins where (flags & " . FLAG_SKIN_DEFAULT . ")!=0");
	    $res = db_query ($query); $result = db_fetch_results ($res);
	    $GLOBALS["skinid"] = $result[0];
	}
	...
	// XXX: we *should* get rid of the cached skin templates...
    }
	...
	if ((db_nof_results ($res) > 0) and ($password == $result[0])) {
	    // yes. the username and password are correct. is this account
	    // enabled?
 ...
 ?><html><head><title><?php
    ...
    eval ("\$tmp = \"" . preg_replace ("/\"/", "\\\"", $title) . "\";"); echo $tmp; ?></title><style type="text/css">
<?php
...
<?php
	...
	$query = sprintf ("select id from privatemessages where userid=%s and not (flags&%s)", $GLOBALS["userid"], FLAG_PM_READ);
	...
	while (list (, $word) = each ($words)) {
		...
		for ($i = 0; $i < strlen ($word); $i++) { $repl .= "*"; }
		    ...
		    $text = preg_replace ("/(\W)(" . $word . ")(\W)/", "\\1" . $repl . "\\3", $text);
	        } else {
		    // no. build the replacement
		    $repl = "";
		    for ($i = 0; $i < strlen ($word); $i++) { $repl .= "*"; }
